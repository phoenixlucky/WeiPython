# 更新日志
## v2.8.7 - 外观设置修复与性能优化 (2026-08-06)

### 🐛 Bug 修复

- ✅ 修复外观设置在随机端口下重启丢失：配置持久化至用户数据目录 `userData/skin.json`，不受本地服务端口变化影响
- ✅ 修复保存失败后的配置丢失路径：`localStorage` 兜底数据优先合并并写回服务端，成功后才清除
- ✅ 修复取色器拖动卡顿：前端内存缓存 + 拖动仅实时预览、松开才保存，不再高频下载壁纸数据
- ✅ 修复大壁纸滚动卡顿：去掉背景层冗余 `background-attachment: fixed`

### 🚀 功能改进

- ✅ 导入壁纸统一自动转为 webp 格式（环境不支持时回退 JPEG），体积更小
- ✅ 外观保存采用增量合并：颜色 / 标语等轻量修改只提交变更字段；服务端写队列串行化防止并发覆盖

### 🧪 测试

- ✅ 新增 `/api/skin` 接口集成测试（5 例）：空配置、增量合并、并发不丢字段、磁盘落盘、无 userData 时 501 降级

---

## v2.8.6 - 外观设置（皮肤系统） (2026-08-06)
...
## v2.8.4 - pip 下载源选择与进程监控 (2026-08-06)

### 🚀 新增功能

- ✅ 包管理新增 pip 下载源选择：官方 PyPI、清华大学、阿里云、中国科大镜像及自定义源
- ✅ 安装、升级、批量升级、升级 pip、按 requirements 安装等操作均通过 `--index-url` 使用所选源
- ✅ 运行日志面板新增进程监控：实时显示活跃进程 PID、运行时长与命令，任务完成后自动停止轮询

### 🐛 交互改进

- ✅ 运行中任务显示 pip 进程 PID 与已等待秒数，长时间无输出时提示切换 pip 源重试
- ✅ 自定义 pip 源地址提交前校验协议（仅允许 http/https）

### 🔧 技术改进

- ✅ 新增 `GET /api/processes` — 查询当前活跃子进程列表
- ✅ `src/utils/process.js` 新增 `getActiveProcesses()`；package 任务快照记录子进程 PID

---

## v2.8.2 - 界面重构与打包流程升级 (2026-08-06)

### 🎨 界面改进

- ✅ 移除运行时横幅与操作弹窗，统一为「运行日志」面板（Runtime Log），所有任务的实时输出集中展示，支持一键清空
- ✅ 导航栏增加图标，hero 区域精简，删除重复的「结果输出」「初始化日志」卡片

### 🔧 工程结构

- ✅ 新增 `一键打包.bat` — Windows 交互式打包入口：读取/设置版本号、选择打包模式（完整安装包 / 免安装目录）
- ✅ 新增 `scripts/set-version.mjs` — 版本号读写与格式校验脚本（支持 `X.Y.Z` 及预发布后缀）
- ✅ 移除遗留 Python 管理脚本（`environment_manager.py`、`python_manager.py`、`run_manager.py` 等）与过时文档，结构更清晰

### 📄 文档

- ✅ README 更新：项目结构、测试命令、安装包文件名

---

## v2.7.4 - Python 升级收尾恢复 (2026-06-22)

### 🐛 Bug 修复

- ✅ Windows 上 Conda 已完成 Python 升级、但清理 `.c~/.conda_trash` 临时文件崩溃时，改为校验实际 Python 版本后判定结果
- ✅ 非清理类错误及实际版本不匹配仍严格判为失败，避免误报成功
